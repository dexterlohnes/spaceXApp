package com.dexterlohnes.kmmnetworkingsample.shared.cache.shared
...
import comdexterlohneskmmnetworkingsamplesharedcache.Rocket
import comdexterlohneskmmnetworkingsamplesharedcache.SelectAllLaunchesInfo
import kotlin.Any
...
private class AppDatabaseQueriesImpl(
  ...
  override fun <T : Any> selectAllLaunchesInfo(mapper: (
    ...
    name: String?,
    type: String?
  ) -> T): Query<T> = Query(-72792594, selectAllLaunchesInfo, driver, "AppDatabase.sq",
  ...
  override fun insertLaunch(
    ...
    notifyQueries(-901353422, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  override fun insertRocket(
    ...
    notifyQueries(-717188269, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllLaunches() {
    driver.execute(-717867880, """DELETE FROM Launch""", 0)
    notifyQueries(-717867880, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllRockets() {
    driver.execute(421164104, """DELETE FROM Rocket""", 0)
    notifyQueries(421164104, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  private inner class SelectRocketByIdQuery<out T : Any>(
    ...
    override fun execute(): SqlCursor = driver.executeQuery(1475558472, """
    |SELECT * FROM Rocket
    |WHERE id = ?
    """.trimMargin(), 1) {
      bindString(1, id)
